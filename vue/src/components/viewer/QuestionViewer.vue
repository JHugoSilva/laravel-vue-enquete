    ...
    <fieldset class="mb-4">
        ...
        <div class="mt-3">
            <div v-if="question.type === 'select'">
                <select :value="modelValue" @change="emits('update:modelValue', $event.target.value)" class="mt-1 w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            ...
            <div v-else-if="question.type === 'radio'">
                <div v-for="option of question.data.options" :key="option.uuid" class="flex items-center">
                    <input :id="option.uuid" @change="emits('update:modelValue', $event.target.value)" :name="'question' +question.id" :value="option.text" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
            ...
            <div v-else-if="question.type === 'checkbox'">
                <div :key="option.uuid" class="flex items-center" v-for="(option) of question.data.options">
                    <input :id="option.uuid" @change="onCheckboxChange" v-model="model[option.text]" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                    <label :for="option.uuid" class="ml-3 block text-sm font-medium text-gray-700">
                        {{ option.text }}
                    </label>
                </div>
